...
- 每个 reason MUST 用自然、通顺的简体中文写清三层内容（不是填空模板）：(1) 当前译法哪里有问题；(2) 有明确改法时给出推荐日语形（可用「」，读音可写在括注；可用 旧形 → 「新形」 这类对比写法嵌入句中）；(3) 为什么必须这样改（对理解或表达有何影响）。说明须让不懂日中翻译技巧、也不一定能读日语的人也能明白；禁止只说“语境不好/不自然”而不解释为什么。仅写 缺少"X"在… 或只标位置、不写为什么，一律不合格。推荐修正写在 reason 内（无单独 suggested 字段）。
...
- 添削対象有多段时：MUST 逐段扫描并尽量覆盖各段真实问题（系统性覆盖），禁止只挑开头一两处就停止；禁止为凑覆盖而编造问题；质量优先于条数。reason/overallComment 宜简明完整，勿空话灌水，也勿因过短而省略“为什么”或推荐改法。
...
suggestions 目标至少 5 条（多段长文时常应更多）；优先检查意义不一致、语法、流畅度、拼写，并兼顾用词/敬语/标点/结构。禁止在仅找到 1–2 条真实问题后提前结束；禁止编造或重复凑数；确实不足 5 条时只返回真实找到的条数。"""

FEW_SHOT_EXAMPLE = """例（中译日文学/学术；教学型指摘，自然中文，非表面润色；勿用「现状：」「推荐：」标签）：
原文：现代人阅读史诗的经验，大概是把它们当作一种印在纸上的文字来读。可实际上，史诗首先是一种声音。
添削対象：現代人が史詩を読む経験は、史詩を紙に印する文字として読む。でも、実際には、史詩はまず声である。

输出：{"suggestions":[{"id":"1","original":"史詩","reason":"「史詩」像未消化的中文词形，宜改为「叙事詩」（じょじし）：日语社科/文学里“史诗”的规范译词是「叙事詩」。混用会暴露领域译词基础不足，后续译文也容易继续写错专名","sourceExcerpt":"史诗"},{"id":"2","original":"でも、","reason":"「でも」偏日常口语转折，读起来像会话，宜改为「しかし」：后者是书面论述转折。本文是学术随笔开篇，应用后者，否则语域掉到口语，影响今后同类论述译的语体判断","sourceExcerpt":"可实际上"},{"id":"3","original":"紙に印する文字","reason":"「紙に印する文字」宜改为「紙に印刷された文字」：原文“印在纸上的文字”指印成的文字成品；「印する」不是自然日语对应，读者难立刻懂是印刷文本——这是词汇/搭配理解问题。不要主推删掉「紙に」之类表面省略","sourceExcerpt":"印在纸上的文字"}],"overallComment":"已能传达“史诗首先是声音、而非只是纸面文字”这一核心对比。主要问题是规范译词、语域与词汇搭配：专名宜用「叙事詩」，论述转折宜用书面语，印刷义宜用自然搭配。"}"""
...
【再确认】reason 与 overallComment 必须是简体中文；禁止日语说明文。中文引用用 "" / “”；日语词形可用「」，禁止用「」包中文说明词。overallComment 先写优点再写问题。每个 reason MUST 用自然中文写清：问题、推荐改法（如有）、为什么必须改（尽量点明对今后翻译能力的影响）；禁止强制「现状：」「推荐：」「現状：」「推奨：」等冒号标签口播。优先实质问题，禁止表面省略当主指摘；禁止无教学的“跟原文对词”；词汇升级须先对比两词语感再推荐。禁止只写位置的 缺少"X"在…。不要臆造不必要的“缺少”助词。对照原文时禁止误引/编造原文；多段时逐段扫描、勿只写 1–2 条就停。只输出 JSON。
