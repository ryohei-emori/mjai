...
**Key pool behavior:** each outbound Groq/Cloudflare/Gemini call selects a credential via round-robin among non-cooled-down entries; on HTTP 401/403/429 the failing credential is cooled down and the next key/pair is tried before the provider fails over. **Groq and Gemini cooldown is model-scoped** (a 429 on one model does not block the same key for a sibling rotation model); Cloudflare cooldown remains credential-wide. Single-key Vercel setups need no change. Plural vars override singular (they are not merged — no double-counting). Duplicate identical keys in a plural list collapse to one entry.

**Cooldown duration** is the provider's own retry hint when it sends one — Groq `retry-after` / `x-ratelimit-reset-requests` (compact form, e.g. `2m59.56s`), Gemini `RetryInfo.retryDelay` inside the 429 body — otherwise the 60s default. Hints are **clamped to 15 minutes** (`provider_health.MAX_COOLDOWN_S`): free-tier *daily* limits report hints measured in hours, and honoring one would withhold a provider until tomorrow even when the hint is wrong or the key has since been replaced. Re-checking every 15 minutes costs one fast 429; being wrong for a day does not. Do not raise the cap to "respect the provider" — the conservative direction is deliberate.

**Cooldowns are shared across invocations** (`route-around-exhausted-providers`, 2026-08) via the `provider_health` table (migration 008). Each request reads one snapshot together with the stored prompt on a single connection and seeds the in-process cooldowns from it, so a fresh Vercel isolate starts out knowing which credentials earlier requests found rate-limited instead of re-collecting one 429 per pooled key — against the quota that is already exhausted. Rows key on `(provider, model, credential_fingerprint)`, hold a **hash prefix and never the key**, and are state rather than history (upserted, tens of rows, no cleanup job). **The table's absence is not an error**: the code falls back to per-process cooldowns, so migration and deploy are order-independent. Reads and writes are bounded (3s read, 2.5s write, write skipped when under ~4s of request budget remains) and never fail a request.
...
**Shared availability does not measure quota.** There is no free pre-flight "how much is left" call on any of the three providers — a probe spends exactly the request it is checking — so `provider_health` records only refusals the providers already reported. It cannot predict the *first* 429 of a limit window, only avoid repeating it. Two guards keep a stale record from becoming an outage: entries expire on their own, and if **every** configured provider looks unavailable the chain still attempts the one recovering soonest (`_plan_providers()`), releasing that cooldown. So "everything is exhausted" always costs one real attempt — by design, since our own cache must never be the sole reason nothing was called. A skipped provider is named in the 503 breakdown (`gemini_error` etc.) as `… every credential was already in cooldown from an earlier request, expected usable in Ns`, which is distinct from `… not configured` and from the budget skip added by `fix-function-invocation-timeout`.

**Availability never reorders providers.** Preference order (Gemini → Groq → Cloudflare) encodes critique quality, and availability only decides which providers are *in* the chain. Do not promote a provider for being faster — Groq answers in 1-3s versus Gemini's 7-16s, and swapping them would trade the output quality several prior changes were spent tuning for a few seconds nobody asked to save. Slow-but-working providers are handled by the phase deadlines, not by demotion.
...
- `frontend/next.config.js` still sets `output: 'export'` (plus `trailingSlash: true`, `images.unoptimized`), so `npm run build` produces a static export in `frontend/out/`. An earlier revision of this file claimed the explicit output mode had been removed — it had not. `frontend/Dockerfile` **is** gone. `frontend/out/` used to carry 18 tracked files from before `out/` was added to `.gitignore`; they were untracked in 2026-08 because a local build overwrote them and left the tree permanently dirty, which made "is production serving a stale build?" needlessly hard to answer. Vercel regenerates the directory from `buildCommand` on every deploy, so nothing depends on it being in git — leave it untracked.
...
- Tables use snake_case columns (`sessions`, `correction_histories`, `ai_proposals`, `app_settings`, `provider_health`); `backend/app/db_helper.py` maps to camelCase for API responses.
...
- **LLM provenance columns** (`007_history_llm_provenance.sql`): `correction_histories.llm_provider` (`gemini` / `groq` / `cloudflare` / `webllm`) and `llm_model` (exact model id). Kept separate from the existing `provider` column, which records the transport (`api` / `webllm`). Rows written before the migration read back `NULL`, and the UI shows no model caption for those.
- **`provider_health`** (`008_provider_health.sql`): when each LLM credential is expected to be usable again, keyed on `(provider, model, credential_fingerprint)` where the fingerprint is a **hash prefix, never the key**. State rather than history — rows are upserted (`GREATEST` on the recovery instant, so concurrent isolates cannot shorten each other's cooldown), cardinality is providers × models × keys, and a row whose `recover_at` has passed is inert, so there is no cleanup job. See the key-pool section above for how it is used.
- **Deploy order is not load-bearing for these three migrations.** Because migrations are applied to the shared project by hand, a deploy can land first, so the code probes for each addition instead of assuming it: `db_helper._has_provenance_columns()` checks `information_schema` once per process and drops `llm_provider` / `llm_model` from every history read and write when they are absent (otherwise a missing column would 500 the whole workspace, not just the model caption); a missing `app_settings` table makes `GET /settings/prompt` serve the built-in default while `PUT` returns a 503 naming `006_app_settings.sql`; and a missing `provider_health` table simply means cooldowns stay per-process, which is how the system behaved before 008. **Still apply all three** — this is a safety net for the window between deploy and migration, not a reason to skip them: until 007 runs no provenance is recorded at all, and until 008 runs every request re-learns the same rate limits.
...
User Request → POST /api/suggestions (authenticated)
 ↓
 One DB read: stored prompt + which credentials earlier requests found rate-limited
 ↓ (providers whose whole pool is known-refused are skipped without a call)
...
|------|----------|---------|-----------|
| **Default** | Gemini (`gemini-3.7-flash` / `gemini-3.6-flash` rotation; pin via `GEMINI_MODEL`) | ~7-16s | Gemini keys configured and not already known to be rate-limited |
| **Secondary** | Groq, model rotation pool (see below), overridable/pinnable via `GROQ_MODEL` | ~1-3s | Gemini skipped for a known limit, or rate-limited/error/timeout/unusable content |
| **Tertiary** | Cloudflare Workers AI | ~2-5s | Gemini and Groq failed, were skipped, or returned unusable content |
...
| `key_pool.py` | Multi-credential load/select/cooldown for Groq + Cloudflare + Gemini (env-driven), plus `pool_availability()` / `release_soonest_cooldown()` used to decide whether a provider is worth calling |
| `provider_health.py` | Shared credential availability: the combined prompt+availability read, cooldown seeding, retry-hint parsing and clamping, and the buffered write back to `provider_health`. Never raises |
...
- **No provider badge**: the クラウドAPI / ローカルAI badge beside the offline toggle was **removed** (`english-ui-labels-and-chrome-polish`). It reported the transport of the *last* generation while each job card already carries its own, so on a queue of several it contradicted whichever card the user was reading. `lastSuggestionSource` is still tracked — it feeds the saved `llmProvider` — it is just no longer rendered there
- **Browser notifications**: an opt-in `Desktop notifications` toggle in the bell panel announces a completed job when the tab is **not** visible (`add-browser-job-notifications`). Permission is requested from that toggle only, never on page load; `denied` and unsupported browsers disable the toggle and leave the in-app bell untouched. Persisted as `mjai-browser-notifications` in `localStorage`
- **UI chrome is English**; AI critique output (Simplified Chinese), user input, and prompt/parser internals (`instructionPrompt`, the `抽出できませんでした` sentinel, `webllm/prompts/templates.ts` section headers) are deliberately untranslated — see `docs/UI-DESIGN.md` → UI Language
...
- **Offline WebLLM applies the stored prompt too** (`revise-system-prompt-settings-ui`, 2026-08). `page.tsx` reads `/settings/prompt` before an offline job and passes it as `PromptInput.systemPromptOverride`; `webllm/prompt.ts buildSystemPrompt()` composes body → exemplar rules → contract, mirroring `build_system_prompt`. Two constraints hold:
  - **Only a customized prompt overrides.** `GET /settings/prompt` returns the cloud default when nothing is stored; passing that through would replace the 7B-tuned offline prompt with ~5,000 characters of cloud rules for every offline user who never touched settings. A failed read falls back the same way — offline mode is where that request is most likely to fail, and a settings outage must not become a generation outage.
  - **This is about which prompt, not when WebLLM runs.** WebLLM is still loaded and called only in explicit オフラインモード. Do not reintroduce automatic fallback from a failed cloud call.
- `frontend/src/lib/webllm/prompts/system.ts` is split into `SYSTEM_PROMPT_HEAD` / `OUTPUT_CONTRACT` / `SYSTEM_PROMPT_TAIL`, reassembled byte-identically into `SYSTEM_PROMPT`. Unlike the backend, the contract sits **mid-prompt** in the default rather than last, because that is where the offline prompt was tuned and measured — moving it would silently change the prompt every uncustomized offline user receives. An override composes `override + contract`, so it replaces `SYSTEM_PROMPT_TAIL` along with the rest of the body; keep anything parse-critical in `OUTPUT_CONTRACT`. Still mirror new rules into the tail in condensed form; do not port the full backend rules into the 7B instruction budget.
- **The editor discloses assembly order, and that disclosure is tested.** `frontend/src/lib/promptComposition.ts` is the single ordered list the dialog renders (your prompt → exemplar rules? → JSON contract → built-in example → SOURCE → EXEMPLAR? → TARGET). `backend/tests/test_prompt_composition_order.py` and `frontend/src/lib/webllm/__tests__/promptComposition.test.ts` assert the real builders match it. **If you reorder either builder, update that list** — the tests will name the step, and the UI is telling operators where their pasted text lands.
