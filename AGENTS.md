...
- `call_gemini_with_rotation()` / `call_groq_with_rotation()` take `allow_model_retry`, which the chain sets False when a sibling-model attempt would leave the next configured provider no room (two Gemini timeouts alone are 44s of 55s). A fresh secondary beats a sibling of the model that just failed.
...
| `suggestions.py` | Failover chain logic (Gemini → Groq → Cloudflare) + wall-clock budget before platform 504; returns the best generated body rather than 503 once any pass produced one |
