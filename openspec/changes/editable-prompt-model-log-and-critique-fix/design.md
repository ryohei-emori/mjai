...
**Live LLM probe (not yet measured — no provider credentials in this environment):**

`.github/workflows/critique-probe.yml` runs the probe in CI so the numbers do not depend on a local Python setup: labelling this pull request `run-critique-probe` (or a manual dispatch) measures the `baseline` / `current` / `custom` conditions and uploads the scored rows. It needs `GEMINI_API_KEYS` (or `GEMINI_API_KEY`) in GitHub Secrets and fails with that instruction when absent. The baseline condition imports `prompts.py` from the pre-change commit, so it is most reliable *before* a squash merge makes that commit unreachable. Wiring was checked locally with a deliberately invalid key: all three conditions compose and post (prompt sizes 4,163 / 5,316 / 5,336 chars, the last being the override path), and every row came back `http_status: 400 API key not valid` — so only the credential is missing.
...
1. **Done (2026-08-16, run 31936101275).** `006_app_settings.sql` and `007_history_llm_provenance.sql` are applied to the shared Supabase project, and the migration history now records 001-007. Both are additive and idempotent, and the new code tolerates the un-migrated schema (see the schema probe below), so deploy order was not load-bearing either way. Applying them required a one-time `migration repair` of 001-005, because those were made in the SQL Editor and left the remote history empty while `001_initial_schema.sql` is not idempotent. That is now settled: `.github/workflows/apply-migrations.yml` applies migrations automatically when a push to `main` touches `backend/supabase/migrations/`, which is Supabase's documented setup, so later schema changes need no operator step as long as nobody edits the remote schema outside those files.
