...
**Prompt resolution per generation.** The `/suggestions` handler reads `app_settings.correction_system_prompt` after the wall-clock deadline is established, with a short timeout and no cache, and passes it as `system_prompt_override` into `build_messages()`. The prompt is composed as *rules body* (the stored text, or the built-in `SYSTEM_PROMPT_BODY`) + *exemplar rules when an exemplar was pasted* + *`OUTPUT_CONTRACT`*. The contract — JSON-only instruction and the `格式：` schema line — is appended by code and is not editable, so an edit can lower critique quality but cannot break parsing. Any settings-read failure or timeout logs a warning and falls back to the default, so a settings outage degrades quality at worst, never availability.

**Offline path.** As of `revise-system-prompt-settings-ui`, offline WebLLM generation uses the same stored prompt: `page.tsx` reads `/settings/prompt` before an offline job and passes the text as `systemPromptOverride` into `buildPrompt`, which composes it the same way the backend does (body → exemplar rules → contract). The override is applied **only when `isCustomized` is true**, and a failed read falls back silently — the built-in offline prompt is condensed for a 7B model, so handing every uncustomized offline user the cloud rules would be a large quality change nobody asked for, and offline mode is exactly where that request is most likely to fail. `frontend/src/lib/webllm/prompts/system.ts` is therefore split into `SYSTEM_PROMPT_HEAD` / `OUTPUT_CONTRACT` / `SYSTEM_PROMPT_TAIL`, reassembled byte-identically into `SYSTEM_PROMPT` for the default case; unlike the backend the contract sits mid-prompt in the default, because that is where it was tuned. This changes which prompt WebLLM uses, **not when WebLLM runs** — it is still loaded and called only in explicit オフラインモード, with no automatic fallback from a failed cloud call.

The prompt editor discloses the assembly order from `frontend/src/lib/promptComposition.ts`, and both builders are asserted against that order (`backend/tests/test_prompt_composition_order.py`, `frontend/src/lib/webllm/__tests__/promptComposition.test.ts`) so the disclosure cannot drift from what is sent.
